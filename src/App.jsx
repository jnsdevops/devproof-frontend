  ...
  async function generate() {
    ...
    try {
      ...
      while (attempts < 80) {
        ...
        if (job.status==='done') {
          const d = job.result
          const merged = {...d.analysis,...d.content, duplicate_alert: d.duplicate_alert, gap_alert: d.analysis?.gap_alert}
        ...
        {phase==='results'&&results&&(
          <div className="fade-up">
            ...
            <div style={{...card,marginBottom:12}}>
              ...
              {/* Gap Detection Alert */}
              {results.gap_alert?.has_gaps&&(
                <div style={{padding:'12px 14px',borderRadius:'var(--r)',fontSize:12.5,
                  marginTop:10,background:'var(--danger-bg)',border:'1px solid var(--danger)'}}>
                  <div style={{fontWeight:700,color:'var(--danger)',marginBottom:6}}>
                    ⚠ Missing stages detected in your {results.gap_alert.pipeline} documentation
                  </div>
                  {(results.gap_alert.critical||[]).map(s=>(
                    <div key={s.id} style={{fontSize:12,color:'var(--danger)',marginBottom:4,paddingLeft:8}}>
                      🔴 <strong>{s.label}</strong> — {s.suggestion}
                    </div>
                  ))}
                  {(results.gap_alert.recommended||[]).map(s=>(
                    <div key={s.id} style={{fontSize:12,color:'var(--warning)',marginBottom:4,paddingLeft:8}}>
                      🟡 <strong>{s.label}</strong> — {s.suggestion}
                    </div>
                  ))}
                </div>
              ...
              {results.duplicate_alert?.has_duplicates&&(
                <div style={{padding:'8px 13px',borderRadius:'var(--r)',fontSize:12.5,
                  marginTop:8,fontWeight:600,background:'var(--warning-bg)',
